...
X0 = np.random.randn(4, n_samples) * 0.5 + np.array([[0], [0], [0], [0]])
y0 = np.zeros((1, n_samples))

# Class 1: centered around (2,2)
X1 = np.random.randn(4, n_samples) * 0.5 + np.array([[2], [2], [2], [2]])
...
df_X = pd.DataFrame(tr_X.T, columns=["x_0", "x_1", "x_2", "x_3"])
df_y = pd.DataFrame(tr_y.T, columns=["y"])
df = pd.concat([df_X, df_y], axis=1)
# print(df)

df.to_csv(str(data_content_folder)+"/binary_classification_data_4_f.csv")